// monsters lie ahead
// taken from a github source. Thank whoever who created this, not me -CK

module.exports = (() => {
  const $snCurrentVersion = 2
  const $smrgchEncodedPrefix = 'ADC'
  ...
  const DecodeDeckString = ($strDeckCode) => {
    if ($strDeckCode.substring(0, $smrgchEncodedPrefix.length) !== $smrgchEncodedPrefix) {
      return false
    }

    let $strNoPrefix = $strDeckCode.substring($smrgchEncodedPrefix.length)
    $strNoPrefix = $strNoPrefix.replace(/-/g, '/')
    $strNoPrefix = $strNoPrefix.replace(/_/g, '=')
    let $decoded = Buffer.from($strNoPrefix, 'base64').toString('binary')// base64_decode( $strNoPrefix );
    // console.log($decoded)
    let $deckBytes = []
  ...
  const ReadSerializedCard = ($data, $indexStart, $indexEnd, $nPrevCardBase, $nOutCount, $nOutCardID) => {
    ...
    if ($bHasExtendedCount) {
      if (!ReadVarEncodedUint32(0, 0, $data, $indexStart, $indexEnd, $nOutCount)) { return false }
    } else {
  ...
  const ParseDeckInternal = ($strDeckCode, $deckBytes) => {
    ...
    if ($snCurrentVersion !== $version && $version !== 1) return false
    ...
    for (let $i = $nCurrentByteIndex[0]; $i < $nTotalCardBytes; $i++) { $nComputedChecksum += $deckBytes[$i] }
...
// let deckCodeInput = 'ADCJWkTZX05uwGDCRV4XQGy3QGLmqUBg4GQJgGLGgO7AaABR3JlZW4vQmxhY2sgRXhhbXBsZQ__'

// console.log('ADCJWkTZX05uwGDCRV4XQGy3QGLmqUBg4GQJgGLGgO7AaABR3JlZW4vQmxhY2sgRXhhbXBsZQ__')
// console.log(CArtifactDeckDecoder.ParseDeck(deckCodeInput))
